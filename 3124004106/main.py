"""论文查重命令行入口：读文件 → 分词 → SimHash → 重复率 → 写答案。"""
...
from simhash import compute_simhash, simhash_rate
from tokenizer import tokenize


def parse_args(argv: list[str]) -> tuple[str, str, str]:
    """校验命令行参数：必须恰好 3 个（原文、抄袭版、答案文件路径）。"""
    if len(argv) != 3:
        raise InvalidArgumentError(
            f"参数数量错误：需要 3 个（原文、抄袭版、答案文件），实际 {len(argv)} 个"
        )
    return argv[0], argv[1], argv[2]


def main(argv: list[str] | None = None) -> int:
    """端到端编排：读 → 分词 → SimHash → 重复率 → 写答案。
    ...
    try:
        orig_path, plag_path, ans_path = parse_args(argv)
